...
var bind = {
    ...
    activate: function() {
        Object.keys(bind).forEach(function(btn_ax, i) {
            //goes through btn or ax
            if (btn_ax != 'activate') {
                Object.keys(bind[btn_ax]).forEach(function(piece, j) {
                    //goes through buttons or left and right axes
                    Object.keys(
                        bind[btn_ax][piece]
                    ).forEach(function(which, k) {
                        //goes through the individual functions
                        console.log(btn_ax + ': ' + piece + ', ' + which);
                        gp[btn_ax + '_bind'](
                            piece,
                            which,
                            bind[btn_ax][piece][which].func
                        );
                    });
                });
            }
