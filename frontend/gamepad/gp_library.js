...
function Gamepad() {
  ...
  this.map = function(id, message) { //maps the gamepad, and the related functions unless they've been defined for the button before
    ...
    if(!gp.ready && message) {
      if(match) {
        message.html("The library recognized the gamepad, but didn't map it... Broken? :/");
      } else {
  ...
  this.get_current = function(message) {
    ...
    if(read != undefined && gp.i_use != undefined) {
      ...
      for(var i = 0; i < gp.b_len; i++) {
          ...
          if(where == "buttons") {
            gp.buttons[name].val = (read[where][cur_index].value == layouts[gp.layout].buttons[i].max)? 1 : 0;
          } else if (where == "axes") {
            if(name[1] == 't') {
              gp.buttons[name].val = (read[where][cur_index] + -layouts[gp.layout].buttons[i].base) / (layouts[gp.layout].buttons[i].max - layouts[gp.layout].buttons[i].base);
            } else {
              gp.buttons[name].val = (read[where][cur_index] == layouts[gp.layout].buttons[i].max)? 1 : 0;
            }
          } else {
