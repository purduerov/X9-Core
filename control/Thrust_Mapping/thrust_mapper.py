...
class ThrustMapperMatrices(object):
    """An array of the different matrices"""

    def __init__(self, currentMapperMatrix, allmatrix, minus_t1, minus_t2, minus_t3, minus_t4, minus_t5, minus_t6,
                 minus_t7,
                 minus_t8, none):
        self.currentMapperMatrix = currentMapperMatrix
        self.matrices = [None] * 10
...
class ThrustMapper(object):
    """Class that implements the functions required to generate the thrust map"""
    ALL = 0
    T1 = 1
    T2 = 2
    T3 = 3
    ...
    def __init__(self):
        ...
        self.mapper_matrices = ThrustMapperMatrices(ThrustMapper.ALL, allmatrix, minus_t1, minus_t2, minus_t3, minus_t4,
                                                    minus_t5, minus_t6, minus_t7, minus_t8, none)

        self.desired_force_vector = Vectors.vect6(0, 0, 0, 0, 0, 0)

        self.pivotPosition = Vectors.vect3(0, 0, 0)
        self.thrust_map = None
    ...
    def calculateThrustMap(self, target_vector=None):
        if target_vector is not None:
            self.desired_force_vector = matrices.Matrices.div6(target_vector, 1024)

        cross_result = matrices.Matrices.cross(self.pivotPosition, self.desired_force_vector.R)
        ...
        self.thrust_map = matrices.Matrices.matMul_86x61(self.mapper_matrices.matrices[self.mapper_matrices.
                                                         currentMapperMatrix], self.desired_force_vector)

    def changeMapperMatrix(self, enabled_thrusters):
        if enabled_thrusters == 255:
            self.mapper_matrices.currentMapperMatrix = ThrustMapper.ALL
        elif enabled_thrusters == 254:
            self.mapper_matrices.currentMapperMatrix = ThrustMapper.T8
        elif enabled_thrusters == 253:
            self.mapper_matrices.currentMapperMatrix = ThrustMapper.T7
        elif enabled_thrusters == 251:
            self.mapper_matrices.currentMapperMatrix = ThrustMapper.T6
        elif enabled_thrusters == 247:
            self.mapper_matrices.currentMapperMatrix = ThrustMapper.T5
        elif enabled_thrusters == 239:
            self.mapper_matrices.currentMapperMatrix = ThrustMapper.T4
        elif enabled_thrusters == 223:
            self.mapper_matrices.currentMapperMatrix = ThrustMapper.T3
        elif enabled_thrusters == 191:
            self.mapper_matrices.currentMapperMatrix = ThrustMapper.T2
        elif enabled_thrusters == 127:
            self.mapper_matrices.currentMapperMatrix = ThrustMapper.T1
        elif enabled_thrusters == 0:
            self.mapper_matrices.currentMapperMatrix = ThrustMapper.NONE
        else:
            self.mapper_matrices.currentMapperMatrix = ThrustMapper.NONE
